...
**Goal:** Agregar una landing page pública de MechaIA que se muestra cuando no hay usuario autenticado, con navbar, hero, problema, cómo funciona, features, precios y CTA final.

**Architecture:** Un único componente `Landing.tsx` con todas las secciones. `App.tsx` agrega un estado `showAuth` para hacer swap entre `<Landing>` y `<Auth>` sin modal. Los botones de planes guardan el plan en `localStorage` para usar cuando Lemon Squeezy esté listo.

**Tech Stack:** React 18, TypeScript, Tailwind CSS v3, Lucide React (ya instalado), Vite.
...
|---|---|---|
| `src/components/Landing.tsx` | Crear | Todas las secciones de la landing |
| `src/App.tsx` | Modificar (línea 260-262) | Agregar estado `showAuth`, mostrar Landing o Auth según corresponda |

---

### Task 1: Integración en App.tsx

**Files:**
- Modify: `src/App.tsx:17-21` (agregar estado), `src/App.tsx:260-262` (cambiar condicional)

- [ ] **Step 1: Agregar import de Landing y estado showAuth**

En `src/App.tsx`, agregar el import de Landing junto a los otros imports (línea ~10):

```tsx
import { Landing } from './components/Landing';
```

En la lista de estados (línea ~17, junto a los otros `useState`), agregar:

```tsx
const [showAuth, setShowAuth] = useState(false);
```

- [ ] **Step 2: Reemplazar el condicional `if (!user)`**

Reemplazar las líneas 260-262:
...
Esperado: error de TypeScript sobre `Landing` no encontrado (todavía no existe el archivo). Está bien — confirma que el import está conectado.

- [ ] **Step 4: Commit**

```bash
git add src/App.tsx
git commit -m "feat: wire Landing component into App auth flow"
```

---

### Task 2: Landing.tsx — esqueleto + Navbar

**Files:**
- Create: `src/components/Landing.tsx`

- [ ] **Step 1: Crear el componente con props y estructura base**

Crear `src/components/Landing.tsx`:
...
function PricingSection({ onSelectPlan, onStartAuth }: { onSelectPlan: (p: 'base' | 'turbo') => void; onStartAuth: () => void }) {
...
Esperado: `✓ built in Xs` sin errores TypeScript.

- [ ] **Step 3: Levantar dev y verificar navbar**

```bash
npm run dev
```

Abrir `http://localhost:5173` sin usuario logueado. Debe verse la navbar fija con logo, "Iniciar sesión" y "Empezar gratis →". Al hacer click en cualquier botón debe mostrarse el Auth. Verificar efecto blur al hacer scroll.

- [ ] **Step 4: Commit**

```bash
git add src/components/Landing.tsx
git commit -m "feat: Landing skeleton with sticky blur navbar"
```
...
**Files:**
- Modify: `src/components/Landing.tsx` — reemplazar `HeroSection` placeholder

- [ ] **Step 1: Reemplazar la función `HeroSection`**

Reemplazar la función `HeroSection` completa por:
...
Con `npm run dev` corriendo, abrir `http://localhost:5173`. Verificar:
- Glow azul sutil en el fondo
- Badge animado con punto pulsante
- Título grande con gradiente azul/índigo en la segunda línea
- Dos botones alineados
- Mockup de chat con mensajes y typing indicator

- [ ] **Step 3: Commit**

```bash
git add src/components/Landing.tsx
git commit -m "feat: landing hero section with chat mockup"
```
...
**Files:**
- Modify: `src/components/Landing.tsx` — reemplazar `ProblemSection` placeholder

- [ ] **Step 1: Agregar imports de Lucide al principio del archivo**

Al inicio de `Landing.tsx`, agregar a los imports:

```tsx
import { AlertCircle, Clock, MessageSquare, FileText, Smartphone, History, Zap, Wrench, CheckCircle } from 'lucide-react';
```

- [ ] **Step 2: Reemplazar la función `ProblemSection`**
...
En el browser, hacer scroll debajo del hero. Verificar las 3 cards con íconos de colores sobre fondo `slate-900`. Verificar que la línea de cierre se ve bien centrada.
...
**Files:**
- Modify: `src/components/Landing.tsx` — reemplazar `HowItWorksSection` placeholder

- [ ] **Step 1: Reemplazar la función `HowItWorksSection`**
...
- [ ] **Step 2: Verificar que el anchor funciona**

En el browser, hacer click en "Ver cómo funciona ↓" del hero. Debe hacer scroll suave hasta esta sección. Verificar los 3 pasos con números grandes decorativos.
...
**Files:**
- Modify: `src/components/Landing.tsx` — reemplazar `FeaturesSection` placeholder

- [ ] **Step 1: Reemplazar la función `FeaturesSection`**
...
En el browser, hacer scroll a esta sección. Verificar el grid de 6 cards (1 col en mobile, 2 en tablet, 3 en desktop). El hover debe cambiar el borde a azul sutil.
...
**Files:**
- Modify: `src/components/Landing.tsx` — reemplazar `PricingSection` placeholder

- [ ] **Step 1: Reemplazar la función `PricingSection`**
...
- [ ] **Step 2: Verificar que el localStorage se guarda correctamente**

En el browser, abrir DevTools → Application → Local Storage. Hacer click en "Empezar con Turbo" desde la landing. Verificar que se guarda `selectedPlan: "turbo"` en localStorage antes de mostrar el Auth.

- [ ] **Step 3: Commit**

```bash
git add src/components/Landing.tsx
git commit -m "feat: landing pricing section with plan selection"
```
...
**Files:**
- Modify: `src/components/Landing.tsx` — reemplazar `CtaSection` y `FooterSection` placeholders

- [ ] **Step 1: Reemplazar `CtaSection`**
...
- [ ] **Step 2: Reemplazar `FooterSection`**
...
- [ ] **Step 3: Verificar la página completa de arriba a abajo**

En el browser, hacer scroll por toda la landing. Checklist:
- [ ] Navbar fija con efecto blur al scrollear
- [ ] Hero con glow, badge, título con gradiente, mockup de chat
- [ ] Sección problema con 3 cards de colores
- [ ] Cómo funciona con números grandes decorativos
- [ ] Features grid 6 cards
- [ ] Precios con 2 planes (Base outline, Turbo gradiente)
- [ ] CTA final con gradiente azul/índigo
- [ ] Footer con logo y copyright
- [ ] Responsive: achicar la ventana a mobile y verificar que todo se ve bien

- [ ] **Step 4: Commit final**

```bash
git add src/components/Landing.tsx
git commit -m "feat: landing CTA and footer — landing page complete"
```
